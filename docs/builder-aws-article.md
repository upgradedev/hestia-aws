...
The visitor opens one URL, clicks once, and gets 30 minutes of access to a stored demo copy of a fictional Athens household. Home opens with a Start here block: report a repair, add an appliance the household owns, or paste a receipt or order email for a second, tool-less agent to read.
...
The decision that matters most is the repair: a Bosch washing machine, a recorded repair of EUR 185.00, and a seller called Acropolis Appliance Store. Deterministic Python on the server prepares the notice to the seller, and reviewing it opens the case file. The person reads the exact text and approves it. Approval consumes a single-use token bound to that draft, is recorded as a simulation, and moves the case to Authorized. No notice or email is sent.
...
A sentinel with nothing to watch is a demo. So the household keeps its own registry through two intake kinds, `appliance` and `repair`. An `appliance` record puts one of the household's own appliances, such as a fridge or a laptop, beside the three sample ones. It carries the receipt reference, seller and guarantee months, plus any product, manual or quick start link the household saved. Reporting a repair marks that appliance's claim open, so the household can prepare the same notice, approval and case file the sample repair uses, and a second repair on it is refused while the claim is open. Both kinds run through the intake contract that was already there: stage a draft, show the exact fields, take consent for the exact subset, then commit.
