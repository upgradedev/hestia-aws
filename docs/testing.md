...
| HE19 reading pasted text | `tests/test_registry_intake.py` reading tests; `frontend/tests/registry.spec.ts` paste journeys | Fails closed without a model, past the cap and on a model failure; identical text replays; proposed records are stored as a staged draft, while only confirmed changes enter household records |
...
`tools/measure.py` runs six detectors on a fixed synthetic fixture against pinned expected values, and `tools/ablate.py` removes one detector at a time on the same facts. Both write new files only and refuse the two historical paths. The original payloads in `docs/measurement.json` and `docs/ablation.json` are retained verbatim under `historical_output`, behind a top-level warning that they are superseded synthetic artifacts and not current release evidence. Their recovery, exposure and delta labels do not establish real impact, seller confirmation, legal entitlement or avoided household spending. `tests/test_measurement_truthfulness.py` checks the overwrite refusal, and `tests/test_claims_inventory.py` checks that both files retain the warning.
