...
SUFFIXES = {
    ".html", ".json", ".js", ".css", ".svg", ".png",
    ".jpg", ".jpeg", ".ico", ".webp", ".woff", ".woff2", ".txt"
}
...
def aws(*args):
    result = subprocess.run(
        ["aws", *args, "--no-cli-pager"], check=False, capture_output=True, text=True
    )
    if result.returncode != 0:
        raise RuntimeError(f"aws failed ({result.returncode}): {result.stderr}")
...
def build_release(directory, sha):
    ...
    manifest = {"commit": sha, "files": {
        k: hashlib.sha256(v).hexdigest() for k, v in sorted(payloads.items())
    }}
    payloads["release.json"] = (json.dumps(manifest, indent=2) + "\n").encode()
    return payloads


def publish(directory, sha, stack, region="eu-west-1", command=aws):
    ...
    if not re.fullmatch(r"(lasttake|merismos|archon|hestia)-web-[0-9]{12}-[a-z0-9-]+", bucket):
    ...
    with tempfile.TemporaryDirectory(prefix="frontend-release-") as tmp:
        ...
        for key in ordered:
            ...
            media_map = {".js": "text/javascript", ".svg": "image/svg+xml", ".css": "text/css"}
            ...
            for target in (f"releases/{sha}/{key}", key):
                command("s3api", "put-object", "--bucket", bucket, "--key", target,
                        "--body", str(path.resolve()), "--content-type", media_type,
                        "--cache-control", cache, "--region", region)


        result = command(
            "cloudfront", "create-invalidation", "--distribution-id", distribution,
            "--paths", "/", "/index.html", "/release.json"
        )
    return {
        "url": outputs["FrontendUrl"],
        "commit": sha,
        "files": len(payloads),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stack", default="hestia-frontend")
    parser.add_argument("--region", default="eu-west-1")
    args = parser.parse_args()
    print(json.dumps(publish(args.directory, args.sha, args.stack, args.region), indent=2))
