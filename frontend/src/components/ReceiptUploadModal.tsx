...
export const ReceiptUploadModal: React.FC<ReceiptUploadModalProps> = ({ isOpen, onClose, outflows, enabled, onReceiptMatched }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      <div role="dialog" aria-modal="true" aria-labelledby="receipt-title" className="max-w-xl w-full bg-[#0d121c] border border-amber-500/30 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="p-6 space-y-4 text-xs text-slate-300">
          <p role="status" className="p-3 rounded-xl border border-amber-500/30 text-amber-300">Receipt scanning is not enabled in this demo. No document was scanned and no asset will be created.</p>
          <input type="file" disabled aria-label="Receipt scan unavailable" className="w-full opacity-50" />
